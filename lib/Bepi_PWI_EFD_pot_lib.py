"""
    BepiColombo Mio PWI EFD Pot: L1 QL -- 2025/7/24
"""
import numpy as np
import math
...
def efd_pot_read(cdf, tlm_mode):
    ...
    if tlm_mode!='h':       # L & M
        data.BIAS_LVL_U1= cdf['BIAS_LVL_U1'][...]       # CDF_REAL4 []
        data.BIAS_LVL_U2= cdf['BIAS_LVL_U2'][...]       # CDF_REAL4 []
        data.BIAS_LVL_V1= cdf['BIAS_LVL_V1'][...]       # CDF_REAL4 []
        data.BIAS_LVL_V2= cdf['BIAS_LVL_V2'][...]       # CDF_REAL4 []
        data.EFD_Vu1_ENA= cdf['EFD_Vu1_ENA'][...]       # CDF_UINT1 []
        data.EFD_Vu2_ENA= cdf['EFD_Vu2_ENA'][...]       # CDF_UINT1 []
        data.EFD_Vv1_ENA= cdf['EFD_Vv1_ENA'][...]       # CDF_UINT1 []
...
def efd_pot_add(data, data1, tlm_mode):
    """
    input:  data, data1
    return: data
    """
...
def efd_pot_shaping(data, cal_mode, tlm_mode, ant_mode):
    """
    input:  data
            cal_mode    [Power]     0: background          1: CAL           2: all
            tlm_mode    l, m, h
            ant_mode    # 0:both    1:U(WPT)    2:V(MEF)
    return: data
    """
    ...
    if tlm_mode != 'l': data.n_dt = data.Vu1.shape[1]
    else:               data.n_dt = 1
